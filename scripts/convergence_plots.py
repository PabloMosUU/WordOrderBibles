...
BOOK_ID_NAME = {40: "Matthew", 41: "Mark", 42: "Luke", 43: "John", 44: "Acts", 66: "Revelation", "fullbible": "Full Bible"}
...
def calculate_entropy(lines: list) -> list:
    """
...
def run_convergence_on_translation(folder: str, translation: str, save_filepath: str, plot_str_ord: bool):
    ...
    with PdfPages(output_pdf_path) as pdf:
        for i in [40, 41, 42, 43, 44, 66, "fullbible"]:
            ...
            entropies = calculate_entropy(l_i_lines)
            plot_entropies(entropies, BOOK_ID_NAME[i], pdf)
...
def plot_str_ord_convergence(folder: str, translation: str, book: str, pdf):
    ...
    entropy_orig = calculate_entropy(convert_mismatcher_file(file_orig))
    entropy_shuffled = calculate_entropy(convert_mismatcher_file(file_shuffled))
    entropy_masked = calculate_entropy(convert_mismatcher_file(file_masked))
    ...
    plot_entropies(d_order, fr"D_order {BOOK_ID_NAME[book]}", pdf)
    plot_entropies(d_structure, fr"D_structure {BOOK_ID_NAME[book]}", pdf)
...
if __name__ == '__main__':
    ...
    save_filepath_in = sys.argv[3]   # File name where output is stored
    plot_ord_str = sys.argv[4]       # Plot D_order and D_structure convergence

    run_convergence_on_translation(mismatcher_output, bible_translation, save_filepath_in, bool(plot_ord_str))
