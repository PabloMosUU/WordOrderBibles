        ...
        if os.path.isfile(file_path):
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    ...
                    for book_id in [40, 41, 42, 43, 44, 66]:
                        count = 0
                        n_char = 0
                        word_count = 0

                        for line in content:
                            if line.startswith(str(book_id)):
                                count += 1
                                n_char += len(line)
                                word_count += len(line.split(" "))

                        subdict_verses[book_id] = count
                        subdict_char[book_id] = n_char
                        subdict_words[book_id] = word_count
                        max_verses_dict[book_id] = max(count, max_verses_dict.get(book_id, 0))
...
def plot_bible_lengths(length_dict: dict, save_filepath: str):
    ...
    with PdfPages(output_pdf_path) as pdf:
        for book in BOOK_ID_NAME.keys():
            verse_list = []
            for _, books in length_dict.items():

                for bible_book, verses in books.items():
                    if bible_book == book:
...
def get_value_from_dict(row, a_dict):
    """
    Helps to get certain values from a dictionary
    """
    return a_dict.get(row['bible'], {}).get(row['book_id'], None)
...
if __name__ == '__main__':
    ...
    bible_dir = sys.argv[2]            # Directory where the different Bible translations are stored
    ...
    filename_verses = create_full_info_file(bible_dir, output_filename)

    if plot_bible_length == "True":
        plot_bible_lengths(filename_verses, plots_save_directory)
