...
class PbcBible(Bible):
    def __init__(self, comment_lines: list, content_lines: list):
        Bible.__init__(self)
        self.comments = IndelibleDict()
        for key, value in comment_lines:
            self.comments[key] = value
        self.content, self.hidden_content = IndelibleDict(), IndelibleDict()
        for key, value, is_commented in content_lines:
            if is_commented:
                self.hidden_content[key] = value
            else:
...
def parse_pbc_bible_lines(lines: list, parse_content: bool) -> PbcBible:
    ...
    return PbcBible(comment_lines, content_lines)

def parse_pbc_bible(filename: str) -> PbcBible:
    with open(filename) as f:
        lines = f.readlines()
    return parse_pbc_bible_lines(lines, parse_content=True)
...
def process_bible(filename: str, corpus: str) -> Bible:
